  ...
  async sync() {
    ...
    this.currentSyncPromise = (async () => {
      try {
        ...
        // 3A. Process POST operations first across models in strict dependency sequence
        for (const instruction of sortedInstructions) {
          ...
          if (modelDef && apiTypes.includes("POST")) {
            await this.processPostOperations(modelDef);
          }
        }

        // 3B. Process PUT operations across models in strict dependency sequence
        for (const instruction of sortedInstructions) {
          const modelName = instruction.object_name;
          const apiTypes = instruction.api_types || [];
          const modelDef = MODEL_REGISTRY[modelName];

          if (modelDef && apiTypes.includes("PUT")) {
              ...
              try {
                ...
                if (db.registrationPayments) {
                  ...
                  for (const rp of matchingPayments) {
                    await db.registrationPayments.update(rp.id, { registrationId: newId });
                  }
                }
              } catch (cascadeErr) {
