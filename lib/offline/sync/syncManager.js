  ...
  async bootstrapInitialData() {
    ...
    try {
      ...
      if (pdfSetting) {
        ...
        if (pdfSetting.framePdfUrl && typeof fetch !== "undefined") {
          try {
            const fRes = await fetch(pdfSetting.framePdfUrl);
            if (fRes.ok) {
              const buf = await fRes.arrayBuffer();
              framePdfBytes = new Uint8Array(buf);
            }
          } catch (fErr) {
            console.warn("[SyncManager] Could not pre-cache frame PDF:", fErr);
          }
        }
